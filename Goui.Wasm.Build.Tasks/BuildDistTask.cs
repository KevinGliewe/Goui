...
namespace Goui.Wasm.Build.Tasks
{
    public class BuildDistTask : Task
    {
        ...
        {
            var GouiPath = refpaths.FirstOrDefault (x => Path.GetFileName (x).Equals ("Goui.dll", StringComparison.InvariantCultureIgnoreCase));
            if (GouiPath == null) {
                Log.LogError ("Goui.dll not included in the project");
                return;
            }
            var GouiAsm = AssemblyDefinition.ReadAssembly (GouiPath);
            var clientJs = GouiAsm.MainModule.Resources.FirstOrDefault (x => x.Name.EndsWith ("Client.js", StringComparison.InvariantCultureIgnoreCase)) as EmbeddedResource;
            if (clientJs == null) {
                Log.LogError ("Goui.dll missing client javascript");
                return;
            }
            var dest = Path.Combine (distPath, "Goui.js");
...
<body>
    ...
    <script type=""text/javascript"">
        ...
        document.addEventListener(""DOMContentLoaded"", function(event) {{
            GouiWasm(""{entryPoint.DeclaringType.Module.Assembly.Name.Name}"", ""{entryPoint.DeclaringType.Namespace}"", ""{entryPoint.DeclaringType.Name}"", ""{entryPoint.Name}"", assemblies);
        }});
    </script>
