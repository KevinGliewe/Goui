...
{
    public class ImageRenderer : ViewRenderer<Xamarin.Forms.Image, Goui.Html.Image>
    {
        bool _isDisposed;
        ...
        {
            if (Control == null) {
                var imageView = new Goui.Html.Image ();
...
#pragma warning disable 1998
        public async Task<string> LoadImageAsync (ImageSource imagesource, CancellationToken cancelationToken = default (CancellationToken), float scale = 1f)
        {
            ...
            if (!string.IsNullOrEmpty (file)) {
                ...
                image = "/images/" + name;
                if (Goui.UI.TryGetFileContentAtPath (image, out var f)) {
                    // Already published
                }
                else {
                    await Task.Run (() => Goui.UI.PublishFile (image, file), cancelationToken);
                }
            }
            return image;
        }
            ...
            if (streamsource?.Stream != null) {
                using (var streamImage = await ((IStreamImageSource)streamsource).GetStreamAsync (cancelationToken).ConfigureAwait (false)) {
                    if (streamImage != null) {
                        ...
                        var hash = Goui.Utilities.Hash (data);
                        var etag = "\"" + hash + "\"";
                        image = "/images/" + hash;
                        if (Goui.UI.TryGetFileContentAtPath (image, out var file) && file.Etag == etag) {
                            // Already published
                        }
                        else {
                            Goui.UI.PublishFile (image, data, etag, "image");
                        }
                    }
                }
            }
